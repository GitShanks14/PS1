...
%         numData*numSym*Tx
        ...
        %size(receivedSignal)
        %RxSignalFull = [RxSignalFull;receivedSignal];
        ...
        RxOFDMEst = reshape(LSEInversion(Tx,Rx,ChGainEst,RxOFDM,variance), numData,1,Rx);
...
%         BER(:,idx) = errorRate(dataTmp(:),receivedData);
    ...
    % Compute error statistics
    BER1(:,idx) = errorRate(data,OutData);
...
%         BER(:,idx) = errorRate(dataTmp(:),receivedData);
    ...
    % Compute error statistics
    BER2(:,idx) = errorRate(data,OutData);
...
%         numData*numSym*Tx
        ...
        % Channel inversion
        RxOFDM = reshape(receivedOFDMData, numData, Tx);
        RxOFDMEst = reshape(LSEInversion(Tx,Rx,chGain,RxOFDM,variance), numData,1,Rx);
...
%         BER(:,idx) = errorRate(dataTmp(:),receivedData);
    ...
    % Compute error statistics
    BER3(:,idx) = errorRate(data,OutData);
...
% Plot line fit
fitBER3 = berfit(EbNo, BER3(1,:));
%semilogy(ax,EbNo, fitBER, 'y');
%hold(ax,'off');
...
%         BER(:,idx) = errorRate(dataTmp(:),receivedData);
    ...
    % Compute error statistics
    BER4(:,idx) = errorRate(data,OutData);
...
% Plot line fit
fitBER4 = berfit(EbNo, BER4(1,:));

set(fig, 'DefaultLegendAutoUpdate', 'on');
semilogy(ax,EbNo, fitBER1, 'g');
semilogy(ax,EbNo, fitBER2, 'b');
semilogy(ax,EbNo, fitBER3, 'y');
semilogy(ax,EbNo, fitBER4, 'c');
%hold(ax,'off');


legend('2x2 MIMOwith LSE channel inversion','2x2 MIMO with LMMSE channel inversion','2x2 MIMO LSE w ideal channel estimate', '2x2 MIMO LSE w ideal channel estimate');
